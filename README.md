...
The standard playtest uses 34-card decks and a 10-card opening/refill hand. Command starts at 20 and carries forward. After each Battle, base recovery is 10, 7, 5, 4, 3, 2, 1, then 0, reduced by Fronts lost and capped at 20. At the start of every turn, draw 1 card automatically; if already at 10 cards, discard 1 first. Then play one card, Maneuver, or Pass when Pass is legal. Playing a card spends its printed Command cost; Maneuver costs 1 Command. There is no generic Draw action and no standard Cycle action. Draw piles persist between Battles and the discard pile reshuffles only when a draw requires an empty deck. Two consecutive Passes end a Battle. After the first Pass, the opponent takes a normal turn; any intervening operation clears the earlier Pass. The player who made the first of the two consecutive Passes starts the next Battle. Stratagems are public and active when played. The four shipped deck templates contain 14 Force-type cards and 6 printed Names. They carry multiple distinct Heroes; each Hero is Unique, may be played as either a Force or a Name, and each side may play only one Hero card from hand per Battle.
