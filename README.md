...
The static browser runs the **same canonical Cython engine** through Pyodide. Its Python wheel is intentionally minimal: cards, rules, game state/actions/engine, heuristic evaluation, `web_api.PlaySession`, and the current Tactical AI only. Browser modes are product-facing `computer` / `hotseat`; the current computer implementation is `HeuristicAgent`. Simulation, telemetry, balance, counterfactual, MCCFR Python tooling, and other search agents are not packaged into browser play. The native `_fast_search` extension still physically contains the bundled search cores for now; splitting that extension is separate cleanup work. `web/browser-engine.mjs` only transports JSON to `web_api.PlaySession`.
...
ISMCTS serious defaults use 100,000 iterations with UCT exploration `c=0.3`, 12 root-belief samples in simulation, cheap rollouts of depth 5, rollout epsilon 0.12, tree reuse enabled, and progressive widening disabled. The canonical AI optimizer treats that baseline plus the five existing structural variants as knockout entrants rather than applying changes sequentially. Alpha-beta is a seventh entrant in the same seeded bracket. With seven entrants, one receives a deterministic first-round bye and the remaining six play three opening fixtures; winners advance through two semifinals and a final, for six fixtures total when no exact tie requires a replay. Losers are discarded. If alpha-beta wins the bracket, the ISMCTS runner-up is retained as the best surviving ISMCTS configuration. Progressive widening remains **off by default and experimental**: `k * sqrt(N + 1)`, with fixed alpha 0.5. There is no supported alpha knob.
...
`make experiments` runs algorithm verification first and uses `systemd-inhibit` while the selected experiment runs. The default `suite` is the AI optimization flow: baseline ISMCTS, tree-cold, PW 0.5, greedy rollout, rollout depth 8, rollout epsilon 0, and alpha-beta form one seven-entrant seeded single-elimination bracket. Winners advance and losers are discarded. The initial seed shuffle is deterministic from the suite seed and the bracket is not reshuffled between rounds. An exact tied fixture is replayed with a fresh seed; repeated ties, manual skips, or failures use a provisional seed-order advance so the bracket can continue, but the suite is then marked NOT READY. Exploration, belief-sample, tree-capacity, and identical-control checks were already resolved separately and are not rerun in the canonical suite. Parallel matchups display a live per-cell results table and aggregate progress under `artifacts/search-benchmark/`. Game/balance optimization remains separate under `make balance`. Experiment variations belong in `EXPERIMENT` / `EXPERIMENT_ARGS`, not new Make targets.
...
| Which AI/search configuration wins the optimization bracket? | `make experiments` | Seeded knockout AI optimization tournament including alpha-beta |
